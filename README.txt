...
to test each separate implementation of the IndexedUnsortedList interface, including testing functionality
...
After compiling, attempt to run using the command: java ListTester

* Depending on the version of the Java runtime installed on the device running the program, it may be necessary
...
were implemented due to the differing structures of each list type. 
...
-IUArrayList: 
  ...
  have to search for the target before doing the rest of their logic. An issue that comes about from this implementation, though, is that any methods that 
  ...
  benefit of ArrayList is its ability to help minimize memory usage. If a list of a certain size is needed, then the list can be created at exactly that size
  ...
  minimizing the number of times growth needs to occur. 
  Shifting is handled by two different helper methods, shiftElementForward(index) and shiftElementBackward(index), that, depending on the function, will appropriately shift
...
-IUSingleLinkedList:
  ...
  a null pointer exception. Handling these edge cases will make sure this does not occur. Common edge cases include removing the first or last element and adding 
  to an empty list, which requires many different references. Also, this list type will use more memory than the ArrayList, as each element in the list is attached to 
...
-IUDoubleLinkedList: 
  The IUDOubleLinkedList is almost the same as the IUSingleLinkedList, but each node now holds a reference to the node before itself. The biggest improvement this allows 
  for is making removeLast() O(1) as the list now knows what the node before the tail is. This means that the previous node can just be made the tail and set the tail's next node back 
  ...
  which is an iterator that has bi-directional movement. It also allows for extra functionality other than moving back and forth; it allows for adding and setting values and getter
  ...
  To start, navigating from the tail for index 90 out of 100 will be significantly faster than navigating from the head.

Each list type has advantages and disadvantages due to its usage of memory and its methods' time complexity. Due to this, not every list is useful for every purpose, even if
they are functionally the same. A list that doesn't need to change will be better suited for an ArrayList, while a list that needs to dynamically change will be better implemented 
using one of the LinkedList implementations. Between the two LinkedList implementations, if the priority is saving as much memory as possible, a singleLinkedList will be better, but 
if removing or adding from the list is expected, a doubleLinkedList will likely be better due to its optimized navigation.

  

TESTING:

For this assignment, a pre-made testing suit was used. As each list and its associated methods were being added, each method that could be used to change the list was tested.
...
also check for fail-fast behavior, iterator functionality, and list iterator functionality. To ensure testing was proper, reading and verifying the expected results was done 
multiple times throughout the continued development of the list implementations and list tester.
By consistently testing throughout development, multiple issues were fixed, such as edge cases that were improperly handled, resulting most of the time in
null pointer exceptions or errors in adding and removing nodes. For example, during development, the add() method consistently just used the addToRear method, which is functionally 
the same, but during the creation of IUDOubleLinkedList, addToRear was used instead by accident. This issue went unnoticed until the end of adding all the methods, but since there 
were still tests failing and they were from that method, it was easy to find the issue and correct it.


DISCUSSION:

The most challenging aspect of development for this project came from translating the IUSingleLinkedList to the IUDOubleLinkedList, as well as making sure to handle all edge cases.
